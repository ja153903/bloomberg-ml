...
def feature_normalization(train, test):
    ...
    """
    # return minmax_scale(train), minmax_scale(test)
    def normalize(data):
        max_val = np.max(data)
        min_val = np.min(data)
        if max_val == min_val:
...
def compute_square_loss(X, y, theta):
    ...
    hypoth_space = theta.T.dot(X)

    loss = hypoth_space - y

    return 1 / N * (loss.T.dot(loss))
...
def compute_square_loss_gradient(X, y, theta):
    ...
    hypoth_space = theta.T.dot(X)A

    loss = hypoth_space - y

    return (2 / N) * (X.T.dot(loss))
...
def compute_regularized_square_loss_gradient(X, y, theta, lambda_reg):
    ...
    regularization_term = lambda_reg * (theta.T.dot(theta))
    squared_loss = compute_squared_loss(X, y, theta) + regularization_term
